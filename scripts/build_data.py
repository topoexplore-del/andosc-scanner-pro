"""
ANDOSC SCANNER PRO — Build Data Pipeline v18.0
...
GROUPS = {
    # ── US CORE ──
    "🎯 Watchlist Core": [
        "VRT","OGN","POWL","ETN","ANET","MPWR","PWR","CAT","FCX",
...
def run_backtest_for_ticker(ticker, hist, ind, fund_data, is_etf=False, regimes=None):
    ...
    for i in range(start_bar, n, 5):
        ...
        for period in HOLD_PERIODS:
            ...
            baseline[period["key"]].append({"pnl": pnl, "win": pnl > 0})
...
def fetch_history(ticker, retries=2):
    """Download 2y of daily candles with simple retry/backoff. Cached."""
...
def main():
    ...
    print(f"ANDOSC SCANNER PRO — Building data at {datetime.now()}")
    groups = GROUPS
    if args.quick:
