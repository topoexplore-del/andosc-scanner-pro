"""
ANDOSC SCANNER PRO — Multi-Layer Alert System v18.0
...
def build_email_html(confirmed, near_miss, timestamp):
    ...
    <div style="background:#12131a;border:1px solid #2a2b36;border-radius:8px;padding:20px;margin-bottom:16px;text-align:center">
        <h1 style="color:#00e676;margin:0;font-size:22px;letter-spacing:2px">🎯 ANDOSC SCANNER PRO</h1>
    ...
    html += f"""<div style="text-align:center;padding:16px;color:#5a5c66;font-size:10px;border-top:1px solid #2a2b36">
        <p style="margin:0">ANDOSC SCANNER PRO v18.0 · Solo LONG · 4-Layer Validation</p>
...
def main():
    ...
    print(f"ANDOSC SCANNER PRO — Alert Check · {timestamp}\n{'='*55}")
    ...
    if regime == 'BEAR':
        ...
        with open(alerts_path, 'w') as f:
            json.dump({'checked_at': timestamp, 'regime': regime_info, 'confirmed': 0,
                       'signals': [], 'watchlist': [],
                       'note': 'Alertas suspendidas: SPY bajo SMA200 (régimen bajista).'}, f, indent=2)
        return
    ...
    alerts_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data', 'alerts.json')
    with open(alerts_path, 'w') as f:
        json.dump({'checked_at': timestamp, 'regime': regime_info, 'confirmed': len(confirmed),
